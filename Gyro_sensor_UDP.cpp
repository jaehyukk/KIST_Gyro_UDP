...
#include <arpa/inet.h> // inet_addr 함수를 사용하기 위해 추가
#include <unistd.h>
...
void sendUDPData(const char *data, size_t dataSize, const char *ip, int port)
{
	...
	{
		cout << "Success creating UDP socket." << endl;
	}
	...
	// Send data over UDP
	ssize_t bytesSent = sendto(udpSocket, data, dataSize, 0, (struct sockaddr *)&destAddr, sizeof(destAddr));
	if (bytesSent < 0)
	{
		cout << "Error sending UDP data." << endl;
	}
	else
	{
		cout << "Success sending UDP data." << endl;
	}
...
int main(int argc, char *argv[])
{
	...
	// const string SensorPort = "/dev/ttyS0";                    // Linux format for physical serial port.
	...
	vs.writeAsyncDataOutputFrequency(20);
...
void asciiAsyncMessageReceived(void *userData, Packet &p, size_t index)
{
	...
	if (p.determineAsciiAsyncType() == VNYPR)
	{
		...
		sendUDPData(jsonString.c_str(), jsonString.size(), "127.0.0.1", 12345); // Change IP and port as needed

		// char data[128]; // Simulated data to send over UDP

		// Now print out the yaw, pitch, roll measurements.
		// Format the simulated YPR data as a string
		// snprintf(data, sizeof(data), "%.3f %.3f %.3f", ypr.x, ypr.y, ypr.z);
		// sendUDPData(data, strlen(data), "127.0.0.1", 12345); // Change IP and port as needed

		cout << "ASCII Async YPR: " << ypr << endl; // here
	}
